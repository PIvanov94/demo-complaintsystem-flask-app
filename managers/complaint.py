...
class ComplaintManager:
    ...
    def create(complaint_data, complainer):
        """
        TODO: Put some documentation for the code!
        """
    ...
    def update(complaint_data, id_):
        complaint_query = ComplaintModel.query.filter_by(id=id_)
        complaint = complaint_query.first()
        if not complaint:
            raise NotFound("The complaint does not exist")
        user = auth.current_user()

        if not user.id == complaint.complainer_id:
            raise NotFound("The complaint does not exist")

        complaint_query.update(complaint_data)
        db.session.add(complaint)
        db.session.flush()
        return complaint
