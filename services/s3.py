...
class S3Service:
    ...
    def upload_photo(self, file_name, object_name):
        try:
            ...
            self.s3.upload_file(file_name, self.bucket_name, object_name, ExtraArgs={'ACL': 'public-read', 'ContentType': f'image/{ext}'})
            return f"https://{self.bucket_name}.s3.{self.region}.amazonaws.com/{object_name}"
        except ClientError:
            raise InternalServerError("AWS is not available at the moment. Please try again later")